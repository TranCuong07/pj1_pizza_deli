...
# async def getAll_product_bycategory (db: session = Depends(get_db)):
...
# lay san pham tu danh sach category
@category_router.get('/product_category/{slug}')
async def getCategory_byMenu (slug: str,db: session = Depends(get_db)):
